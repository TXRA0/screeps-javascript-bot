...
* ------------------------------------------------------------
 * 🕷️ Spider Road: Cross-Room Automatic Road Planning System (Standalone Version)
 *
 * Features:
 * 1. Persistent Tasks: Memory.roadPlanner stores tasks, supports vision waiting
 * 2. Smart Reuse: Automatically reuses planned paths, efficiency first
 * 3. Dual-Layer Visualization: Room + Map synchronized display (supports continuous lines)
 * 4. Test Mode: Dynamic planning following flags
 *
 * Usage:
 * 1. Place this file in your project
 * 2. Call `mountSpiderRoad()` in main.ts or loop
 * 3. Use `spiderRoad.testMode()` in console to enable testing
 * ------------------------------------------------------------
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.mountSpiderRoad = exports.PlannerResult = void 0;
exports.planRoad = planRoad;
exports.runRoadTasks = runRoadTasks;
...
})(PlannerResult || (exports.PlannerResult = PlannerResult = {}));
/* ===================== Memory Initialization ===================== */
if (!Memory.roadPlanner)
    Memory.roadPlanner = {};
/* ===================== Utility Functions ===================== */
function hashPos(p) {
...
function getAutoFromPos(roomName, targets) {
    ...
    if (!room)
        throw new Error(`no vision of ${roomName}`);
    const base = room.storage || room.find(FIND_MY_SPAWNS)[0] || room.controller;
    if (!base)
        throw new Error(`no base structure in ${roomName}`);
...
function ensureVision(roomNames, fromRoom) {
    ...
    const observers = (Memory['observer'] || [])
...
function executeTask(fromRoom, task) {
    ...
    if (Game.cpu.bucket < cpuBucketMin)
        return PlannerResult.CPU_LIMIT;
    // Initialize runtime state
    if (!task.completedTargets)
        task.completedTargets = [];
    if (!task.cachedPaths)
        task.cachedPaths = [];
    const fromPos = task.fromPos === 'auto'
    ? getAutoFromPos(fromRoom, task.targets)
    : (task.fromPos instanceof RoomPosition
        ? task.fromPos
        : new RoomPosition(task.fromPos.x, task.fromPos.y, task.fromPos.roomName));
    if (!task.targets.length)
        return PlannerResult.NO_PATH;
    // Check vision
    ...
    // Sort by distance
    const sortedTargets = [...task.targets]
    .map(t => new RoomPosition(t.x, t.y, t.roomName))
    .sort((a, b) => a.getRangeTo(fromPos) - b.getRangeTo(fromPos));
    let hasNewPath = false;
    for (const target of sortedTargets) {
        const targetHash = hashPos(target);
        // Skip completed
        if (task.completedTargets.includes(targetHash))
            continue;
        ...
        // Pathfinding
        const result = PathFinder.search(fromPos, { pos: target, range }, {
            plainCost: 4, swampCost: 24, maxOps: 20000, roomCallback: buildCostMatrix
        });
        ...
        // Build
        if (build && visionReady)
            buildRoad(result.path);
    }
    // Visualization
...
function buildRoad(path) {
    ...
    for (const step of path) {
        ...
        if (structureAt(pos) || constructionSiteExists(pos))
            continue;
...
function planRoad(fromRoom, config) {
    Memory.roadPlanner[fromRoom] = Object.assign(Object.assign({}, config), { completedTargets: [], cachedPaths: [] });
}
...
function showTasks() {
    ...
    for (const [fromRoom, task] of tasks) {
        const progress = `${((_a = task.completedTargets) === null || _a === void 0 ? void 0 : _a.length) || 0}/${task.targets.length}`;
...
const mountSpiderRoad = () => {
    ...
    global.spiderRoad = {
        ...
        testMode: () => {
            const startFlag = Game.flags['startingPos'];
            if (!startFlag)
                return console.log('❌ startingPos flag not found');
...
exports.mountSpiderRoad = mountSpiderRoad;
